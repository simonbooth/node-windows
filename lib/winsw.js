module.exports = {
  ...
  createExe: function(name,dir,callback){
    ...
    var originExe = p.join(__dirname,'..','bin','winsw','x'+(require('os').arch().indexOf('64')>0 ? '64':'86'),'winsw.exe'),
        destExe = p.join(dir,name.replace(/[^\w]/gi,'').toLowerCase()+'.exe'),
        originConfig = p.join(__dirname,'..','bin','winsw','x'+(require('os').arch().indexOf('64')>0 ? '64':'86'),'winsw.exe.config'),
        destConfig = p.join(dir,name.replace(/[^\w]/gi,'').toLowerCase()+'.exe.config');
        data = fs.readFileSync(originExe,{encoding:'binary'});

    fs.writeFileSync(destExe,data,{encoding:'binary'});
        data = fs.readFileSync(originConfig,{encoding:'binary'});
      fs.writeFileSync(destConfig,data,{encoding:'binary'});
    callback && callback();
